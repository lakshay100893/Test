...
theme.Cart = (function () {
    ...
    function Cart(container) {
        ...
        var plusCartQTY = document.querySelector(selectors.itemInputPlus);
        var minusCartQTY = document.querySelector(selectors.itemInputMinus);
        minusCartQTY.addEventListener("click", this._updateQuantity);
        plusCartQTY.addEventListener("click", this._updateQuantity);
    ...
    Cart.prototype = Object.assign({}, Cart.prototype, {
        ...
        _handleInputQty: function (evt) {
            ...
            //var minusCartQTY = document.querySelector(selectors.itemInputMinus);
            //var input  =  minusCartQTY.nextElementSibling;
...
(function () {
    ...
    var numberPlace = document.querySelector(".quantity-input");
    var min = 1; /// min number
    var button = document.querySelector(".product-form__cart-submit");

    var productErrorDiv = document.querySelector(
        ".product-form__error-message-wrapper"
    );
    var productErrorMessageSpan = document.querySelector(
        ".product-form__error-message"
    );

    if (minusBtn && plusBtn && numberPlace && button) {
        var max = numberPlace.getAttribute("max"); /// max number
        minusBtn.addEventListener("click", function () {
            ...
            if (number > min) {
                ...
                button.disabled = number < min ? true : false;
                if (productErrorDiv) {
                    if (
                        !productErrorDiv.classList.contains(
                            "product-form__error-message-wrapper--hidden"
                        )
                    ) {
                        productErrorDiv.classList.add(
                            "product-form__error-message-wrapper--hidden"
                        );
                    }
                }
                ...
                if (productErrorDiv) {
                    if (
                        ...
                        productErrorDiv.classList.remove(
                            "product-form__error-message-wrapper--hidden"
                        );
                    }
                    if (productErrorMessageSpan) {
                        productErrorMessageSpan.innerHTML =
        ...
        plusBtn.addEventListener("click", function () {
            ...
            if (number < max) {
                number = number + 1;
                numberPlace.value = number; /// Display the value in place of the number
                button.disabled = number > max ? true : false;
                if (productErrorDiv) {
                    if (
                        !productErrorDiv.classList.contains(
                            "product-form__error-message-wrapper--hidden"
                ...
                if (productErrorDiv) {
                    ...
                    if (productErrorMessageSpan) {
                        productErrorMessageSpan.innerHTML =
                            "Not enough items available. Only " +
                            max +
                            " left.";
                    }
                }
        ...
        numberPlace.addEventListener("focusout", function () {
            var number = parseInt(numberPlace.value);
            button.disabled = false;
            if (number < max) {
                number = this.value;
...
var xcart__submit = document.querySelector(".cart__submit");

if (xcart__submit) {
    setTimeout(() => {
        var xCheckBox = document.querySelector(
            '.rale-checkbox input[type="checkbox"]'
        );

        if (xCheckBox) {
            xcart__submit.disabled = true;
            xCheckBox.addEventListener("change", function () {
                console.log("disabled");
                xcart__submit.disabled = true;
                if (xCheckBox.checked) {
                    xcart__submit.disabled = false;
                }
            });
        }
    }, 2500);
...
setInterval(function () {
    var butNowBut = document.querySelector(".rale-dynamic--hidden");
    if (butNowBut) {
        butNowBut.classList.remove("rale-dynamic--hidden");
    }
}, 1);
